...
```
Client  (Qt5 GUI, WinSock2)  ──→  Server  (IOCP/select, ThreadPool)
                                     └── 存储目录 (.storage/)
                                     └── 日志目录 (logs/)
                                     └── 用户配置 (users.ini)
```
...
- **服务端**: 基于 `select` 的 I/O 多路复用 + 线程池处理任务，支持多客户端并发

## 功能

- 用户登录认证（用户名/密码）
- 文件上传（5MB 分片传输，CRC32 校验）
- 文件下载（5MB 分片传输）
- 文件上传与下载进度条
- 文件列表刷新
- 服务端日志记录（文件 + 控制台）
...
| ---------------------- | ---- | --------------------------------- |
| `MSG_LOGIN`          | C→S | 登录请求                          |
| `MSG_UPLOAD_REQ`     | C→S | 上传请求（含文件名、大小、CRC32） |
